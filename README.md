# Daily Stock Trend Watch

미국/한국 주식의 52주 신고가, 거래량 급증, 한국 외국인/기관 수급, 유명기관 SEC 13F 보유 증감, 컨센서스와 리포트 메타데이터를 매일 자동 수집해 정적 웹 리포트로 배포하는 시스템입니다.

이 시스템은 투자 추천이 아니라 후보 발굴 도구입니다. 원천 데이터가 없거나 파싱되지 않은 필드는 빈칸 또는 `null`로 둡니다. 기관의 매수 이유, 미래 전망, 사업 설명은 임의로 작성하지 않습니다.
...
- `data/processed/latest.json`: 최신 정제 데이터
- `data/processed/latest.csv`: 최신 정제 CSV
- `data/processed/daily_summary.csv`: 일별 섹션별 추적 요약
- `reports/YYYY-MM-DD/report.html`: 날짜별 HTML 리포트
- `reports/report.html`: 최신 HTML 리포트
- `reports/latest.json`, `reports/latest.csv`, `reports/latest.xlsx`: 웹 다운로드용 파일
