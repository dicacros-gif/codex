...
:root{{--bg:#F5F7FA;--card:#FFFFFF;--card2:#F0FDF4;--hdr:#1A3A2A;--hdrText:#FFFFFF;--row2:#F2F6FC;--t1:#111827;--t2:#475569;--t3:#64748B;--bd:#BBF7D0;--grid:#DDE7F3;--ac:#16A34A;--acL:#DCFCE7;--acT:#15803D;--hoverRing:#86EFAC;--okB:#BBF7D0;--okT:#14532D;--buyB:#DCFCE7;--buyT:#166534;--warnB:#FEE2E2;--warnT:#991B1B;--negB:#FECACA;--negT:#7F1D1D;--link:#0563C1;--tbg:rgba(255,255,255,.92);--r:12px;--shadow:0 4px 18px rgba(15,23,42,.08);--scroll:#86EFAC;--scrollTrack:#ECFDF5}}
...
.topbar{{position:sticky;top:0;z-index:50;background:var(--tbg);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border-bottom:1px solid var(--bd);padding:.35rem .9rem;display:flex;align-items:center;gap:.55rem;overflow:hidden;scrollbar-width:thin;scrollbar-color:var(--scroll) transparent}}
.tabs{{display:flex;gap:.2rem;overflow-x:auto;flex:1 1 auto;min-width:0;width:100%;max-width:100%;scrollbar-width:thin;scrollbar-color:var(--scroll) transparent}}.tab-btn{{border:1px solid var(--bd);background:var(--card);color:var(--t2);border-radius:999px;padding:.28rem .65rem;font-size:.68rem;font-weight:900;white-space:nowrap;cursor:pointer;transition:background .18s,border-color .18s,color .18s,box-shadow .18s}}.tab-btn:hover{{background:var(--acL);border-color:var(--ac);color:var(--acT)}}.tab-btn.on{{background:var(--ac);border-color:var(--ac);color:#fff;box-shadow:0 2px 10px rgba(22,163,74,.20)}}
...
table{{width:max-content;min-width:100%;border-collapse:separate;border-spacing:0;font-size:.68rem;line-height:1.3}}th{{position:sticky;top:0;z-index:3;background:var(--hdr);color:var(--hdrText);padding:.30rem .34rem;text-align:left;font-weight:900;white-space:nowrap;border-bottom:2px solid rgba(255,255,255,.22);border-right:1px solid rgba(255,255,255,.14)}}th.sortable{{cursor:pointer;user-select:none}}th.sortable::after{{content:'↕';font-size:.58rem;margin-left:.22rem;color:#BBF7D0}}th.sortable[data-dir='desc']::after{{content:'↓'}}th.sortable[data-dir='asc']::after{{content:'↑'}}td{{padding:.20rem .30rem;border-bottom:1px solid var(--grid);border-right:1px solid var(--grid);vertical-align:top;background:var(--card);color:var(--t1);transition:filter .12s,box-shadow .12s}}tbody tr:nth-child(even) td{{background:var(--row2)}}tbody tr:hover td{{filter:saturate(1.04) brightness(.995);box-shadow:inset 0 1px 0 var(--hoverRing),inset 0 -1px 0 var(--hoverRing)}}tbody tr:hover td:first-child{{box-shadow:inset 4px 0 0 var(--hoverRing),inset 0 1px 0 var(--hoverRing),inset 0 -1px 0 var(--hoverRing)}}tbody tr:hover a{{color:#034EA2!important;text-decoration:underline}}[data-t=dark] tbody tr:hover a{{color:#BFDBFE!important}}.empty{{padding:1.2rem;color:var(--t3);font-size:.82rem;font-weight:800}}.num{{font-family:'JetBrains Mono',monospace;white-space:nowrap;font-weight:650}}.score-cell,.valuation-cell,.growth-cell,.quality-cell,.momentum-cell,.liquidity-cell,.size-cell,.ownership-cell{{background:#F8FAFC!important;color:#334155!important;font-weight:700}}.risk-cell{{background:#FEF2F2!important;color:#991B1B!important;font-weight:750}}.pos-strong,.volume-hot,.flow-pos{{background:#BBF7D0!important;color:#14532D!important;font-weight:820;border-radius:4px}}.pos-buy,.volume-watch{{background:#DCFCE7!important;color:#166534!important;font-weight:800;border-radius:4px}}.warn{{background:#FEE2E2!important;color:#991B1B!important;font-weight:800;border-radius:4px}}.neg{{background:#FECACA!important;color:#7F1D1D!important;font-weight:820;border-radius:4px}}.basis{{min-width:460px;max-width:760px;white-space:normal}}.bullets{{display:grid;gap:.05rem;line-height:1.24;font-size:.64rem}}.basis-cell .bullets{{max-height:none;overflow:visible}}.bullets span{{display:block;padding:.035rem .16rem .035rem .66rem;position:relative;border-left:2px solid transparent;border-radius:4px;font-weight:400}}.bullets span::before{{content:'-';position:absolute;left:.2rem;color:var(--ac);font-weight:700}}.bullets .b-hot{{background:#DCFCE7;border-left-color:#16A34A}}.bullets .b-flow{{background:#ECFDF5;border-left-color:#16A34A}}.bullets .b-good{{background:#DCFCE7;border-left-color:#16A34A}}.bullets .b-tech{{background:#F0FDF4;border-left-color:#22C55E}}.bullets .b-risk{{background:#FEE2E2;border-left-color:#DC2626}}.kw{{font-weight:850}}.kw-hot{{color:#15803D}}.kw-flow{{color:#047857}}.kw-good{{color:#166534}}.kw-tech{{color:#16A34A}}.kw-risk{{color:#DC2626}}.tag{{display:inline-block;border:1px solid var(--bd);background:var(--card2);border-radius:999px;padding:.04rem .34rem;margin:.04rem;font-size:.64rem;font-weight:800;color:var(--t2)}}footer{{font-size:.66rem;color:var(--t3);text-align:center;padding:.8rem 0}}
.key-cell{{background:#F0FDF4!important;color:#14532D!important;font-weight:850}}.company-cell{{background:#FFFFFF!important;color:#111827!important;font-weight:700;font-size:.68rem;max-width:92px;min-width:64px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}.country-us{{background:#F8FAFC!important;color:#334155!important;font-weight:800}}.country-kr{{background:#ECFDF5!important;color:#166534!important;font-weight:800}}.signal-cell{{background:#F8FAFC!important;color:#334155!important;max-width:126px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}.theme-cell{{background:#F0FDF4!important;color:#166534!important;font-weight:780;max-width:110px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}.basis-cell{{background:#FAFAFA!important;color:#334155!important;font-weight:400}}.report-cell{{background:#EFF6FF!important;color:#0563C1!important;font-weight:750;max-width:190px;min-width:108px}}.report-cell a{{display:block;max-width:176px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}}.metric-blue{{background:#EFF6FF!important;color:#1D4ED8!important;font-weight:780}}.metric-cyan{{background:#ECFEFF!important;color:#0E7490!important;font-weight:780}}.metric-purple{{background:#F5F3FF!important;color:#6D28D9!important;font-weight:780}}.metric-soft{{background:#F8FAFC!important;color:#475569!important;font-weight:700}}.section-cell{{background:#ECFDF5!important;color:#047857!important;font-weight:820}}.good-1{{background:#F0FDF4!important;color:#166534!important;font-weight:720}}.good-2{{background:#ECFDF5!important;color:#166534!important;font-weight:740}}.good-3{{background:#DCFCE7!important;color:#166534!important;font-weight:760}}.good-4{{background:#BBF7D0!important;color:#14532D!important;font-weight:800}}.good-5{{background:#86EFAC!important;color:#064E3B!important;font-weight:830}}.bad-1{{background:#FEF2F2!important;color:#B91C1C!important;font-weight:720}}.bad-2{{background:#FEE2E2!important;color:#991B1B!important;font-weight:750}}.bad-3{{background:#FECACA!important;color:#991B1B!important;font-weight:780}}.bad-4{{background:#FCA5A5!important;color:#7F1D1D!important;font-weight:810}}.bad-5{{background:#F87171!important;color:#7F1D1D!important;font-weight:830}}[data-t=dark] .good-1{{background:#0D2218!important;color:#86EFAC!important}}[data-t=dark] .good-2{{background:#11351F!important;color:#BBF7D0!important}}[data-t=dark] .good-3{{background:#14532D!important;color:#D1FAE5!important}}[data-t=dark] .good-4{{background:#166534!important;color:#DCFCE7!important}}[data-t=dark] .good-5{{background:#15803D!important;color:#F0FDF4!important}}[data-t=dark] .bad-1{{background:#3A1515!important;color:#FCA5A5!important}}[data-t=dark] .bad-2{{background:#4C1D1D!important;color:#FECACA!important}}[data-t=dark] .bad-3{{background:#7F1D1D!important;color:#FECACA!important}}[data-t=dark] .bad-4{{background:#991B1B!important;color:#FEE2E2!important}}[data-t=dark] .bad-5{{background:#B91C1C!important;color:#FEF2F2!important}}
@media(max-width:768px){{main{{width:calc(100% - .5rem)}}.topbar{{padding:.28rem .45rem;align-items:flex-start;flex-direction:column}}.tabs{{align-self:stretch}}.tools{{align-self:flex-end}}.table-wrap{{max-height:none}}}}
</style>
</head>
<body data-t="light">
<div class="topbar">
...
function centerActiveTab(btn){{
  if(!tabStrip||!btn) return;
  const targetLeft=btn.offsetLeft-(tabStrip.clientWidth-btn.offsetWidth)/2;
  tabStrip.scrollLeft=Math.max(0,targetLeft);
}}
...
centerActiveTab(document.querySelector('.tab-btn.on'));
...
def _xlsx_style(tokens: list[str]) -> tuple[str, str, bool] | None:
    palette = {
        "good-1": ("F0FDF4", "166534", True),
        "good-2": ("ECFDF5", "166534", True),
        "good-3": ("DCFCE7", "166534", True),
        "good-4": ("BBF7D0", "14532D", True),
        "good-5": ("86EFAC", "064E3B", True),
        "bad-1": ("FEF2F2", "B91C1C", True),
        "bad-2": ("FEE2E2", "991B1B", True),
        "bad-3": ("FECACA", "991B1B", True),
        "bad-4": ("FCA5A5", "7F1D1D", True),
        "bad-5": ("F87171", "7F1D1D", True),
        "pos-strong": ("BBF7D0", "14532D", True),
        "pos-buy": ("DCFCE7", "166534", True),
        "warn": ("FEE2E2", "991B1B", True),
        "neg": ("FECACA", "7F1D1D", True),
        "volume-hot": ("BBF7D0", "14532D", True),
